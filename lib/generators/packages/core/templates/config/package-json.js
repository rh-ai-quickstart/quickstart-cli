...
import { getScriptCommand, runWorkspaceScript, getSetupCommand, } from '../../../../utils/package-manager.js';
export const generateRootPackageJson = (params) => {
    const { config } = params;
    ...
    const packageJson = {
        name: config.name,
        version: '0.1.0',
        description: config.description || `A full-stack application built with modern tools and best practices.`,
        private: true,
        workspaces: ['packages/*'],
        scripts: {
            postinstall: 'husky',
