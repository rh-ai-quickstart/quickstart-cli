export const generateRootReadme = (params) => {
    const { config } = params;
...
- **Turborepo** - High-performance build system for the monorepo${hasUi ? '\n- **React + Vite** - Modern frontend with TanStack Router' : ''}${hasApi ? '\n- **FastAPI** - Python backend with async support' : ''}${hasDb ? '\n- **PostgreSQL** - Database with Alembic migrations' : ''}
...
${config.name}/
├── packages/
${hasUi ? '│   ├── ui/           # React frontend application' : ''}${hasApi ? '\n│   ├── api/          # FastAPI backend service' : ''}${hasDb ? '\n│   └── db/           # Database and migrations' : ''}
...
${hasUi ? '- **Frontend App**: http://localhost:3000' : ''}${hasApi
...
- [Turborepo](https://turbo.build/) - Monorepo build system${hasUi ? '\n- [TanStack Router](https://tanstack.com/router) - Type-safe routing' : ''}${hasApi ? '\n- [FastAPI](https://fastapi.tiangolo.com/) - Modern Python web framework' : ''}${hasDb ? '\n- [Alembic](https://alembic.sqlalchemy.org/) - Database migrations' : ''}
