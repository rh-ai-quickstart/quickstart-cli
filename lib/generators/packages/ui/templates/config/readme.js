export const generateReadme = (params) => {
    const { config } = params;
    const hasApi = config.features.api;
    return `# ${config.name} UI
...
${hasApi ? `- **API Integration**: Service layer with hooks for type-safe API calls` : ''}
...
├── services/                   # API service layer${hasApi ? ' (when API enabled)' : ''}
│   └── health.ts              # Example: Health check service
├── hooks/                      # Custom React hooks${hasApi ? ' (when API enabled)' : ''}
│   └── health.ts              # Example: useHealth hook
├── schemas/                    # Zod schemas for API validation${hasApi ? ' (when API enabled)' : ''}
...
- **Pairing**: One hook file per service file, hooks pair with their corresponding services`
        : `API integration is not available. Enable the API package to add backend integration.`}
