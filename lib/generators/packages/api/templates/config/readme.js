export const generateReadme = (params) => {
    const { config } = params;
    const hasDb = config.features.db;
...
├── models/              # Database models${hasDb ? ' (SQLAlchemy)' : ''}
...
${hasDb ? '5. **Database Sessions**: Use dependency injection for database sessions' : ''}
...
${hasDb ? '- Database transaction rollback for test isolation' : ''}
