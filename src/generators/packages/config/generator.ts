...
export class ConfigPackageGenerator {
  ...
  private async generateESLintConfig(configsDir: string): Promise<void> {
    const eslintDir = path.join(configsDir, 'eslint');

    // Generate package.json
    await fs.outputFile(
      path.join(eslintDir, 'package.json'),
      generateESLintPackageJson(this.config)
    );
  ...
  private async generatePrettierConfig(configsDir: string): Promise<void> {
    const prettierDir = path.join(configsDir, 'prettier');

    // Generate package.json
    await fs.outputFile(
      path.join(prettierDir, 'package.json'),
      generatePrettierPackageJson(this.config)
    );
  ...
  private async generateRuffConfig(configsDir: string): Promise<void> {
