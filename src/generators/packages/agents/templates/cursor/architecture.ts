...
export const generateCursorArchitecture = (params: AgentTemplateParams): string => {
...
└── Makefile             # Common development commands`;
  ...
  const devUrls = [];
  if (features.ui) devUrls.push('- **Frontend**: http://localhost:3000');
  if (features.api) {
    devUrls.push('- **API**: http://localhost:8000');
    devUrls.push('- **API Docs**: http://localhost:8000/docs (Swagger UI)');
  }
  if (features.db) devUrls.push('- **Database**: postgresql://localhost:5432');
