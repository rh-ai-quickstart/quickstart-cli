...
export const generateClaudeCodeStyle = (params: AgentTemplateParams): string => {
...
${features.ui ? `- **UI files**: Prettier + ESLint\n` : ''}${features.api || features.db ? `- **Python files**: Ruff format + Ruff check\n` : ''}
...
` : ''}${features.api || features.db ? `- Python
- Ruff
` : ''}
...
${features.ui ? `  "editor.defaultFormatter": "esbenp.prettier-vscode",\n` : ''}${features.api || features.db ? `  "[python]": {
