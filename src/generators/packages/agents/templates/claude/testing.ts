...
export const generateClaudeTesting = (params: AgentTemplateParams): string => {
...
|---------|-----------|----------|
${features.ui ? `| UI | Vitest + React Testing Library | \`packages/ui/src/**/*.test.tsx\` (co-located) |\n` : ''}${features.api ? `| API | Pytest | \`packages/api/tests/\` |\n` : ''}${features.db ? `| DB | Pytest | \`packages/db/tests/\` |\n` : ''}
...
\`\`\`
` : ''}
${features.api ? `## API Testing (Pytest)
...
# Specific packages
${features.ui ? `pnpm --filter ui test\n` : ''}${features.api ? `pnpm --filter api test\n` : ''}\`\`\`
`;
};
