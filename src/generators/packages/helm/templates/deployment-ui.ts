...
/**
 * Generates UI deployment template
 */
export function generateDeploymentUi(params: HelmTemplateParams): string {
  const { config } = params;
...
spec:
  ...
  selector:
    matchLabels:
      {{- include "${chartName}.ui.selectorLabels" . | nindent 6 }}
  template:
    metadata:
      labels:
        {{- include "${chartName}.ui.selectorLabels" . | nindent 8 }}
    spec:
      ...
      containers:
        - name: ui
          ...
          image: {{ include "${chartName}.image" (dict "name" .Values.ui.image.repository "tag" .Values.ui.image.tag "Values" .Values) }}
