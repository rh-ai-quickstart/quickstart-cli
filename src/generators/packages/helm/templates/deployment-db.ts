...
/**
 * Generates Database deployment template
 */
export function generateDeploymentDb(params: HelmTemplateParams): string {
  const { config } = params;
...
spec:
  ...
  selector:
    matchLabels:
      {{- include "${chartName}.database.selectorLabels" . | nindent 6 }}
  template:
    metadata:
      labels:
        {{- include "${chartName}.database.selectorLabels" . | nindent 8 }}
    spec:
