...
/**
 * Generates Database migration Job template
 */
export function generateMigrationJob(params: HelmTemplateParams): string {
  const { config } = params;
...
metadata:
  ...
  labels:
    {{- include "${chartName}.migration.labels" . | nindent 4 }}
  annotations:
...
spec:
  ...
  template:
    metadata:
      labels:
        {{- include "${chartName}.migration.selectorLabels" . | nindent 8 }}
    spec:
